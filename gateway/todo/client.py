...
class TodoClient:
    ...
    async def create_todo(self, title, description, user_id):
        request = pb2.CreateTodoRequest(
            title=title, description=description, userID=user_id
        )
        return self.stub.CreateTodo(request)

    async def get_todo(self, todo_id, user_id):
        request = pb2.GetTodoRequest(itemID=todo_id, userID=user_id)
        return self.stub.GetTodo(request)

    async def get_todos(self, user_id):
        request = pb2.GetTodosRequest(userID=user_id)
        return self.stub.GetTodos(request)

    async def update_todo(self, id, title, description):
